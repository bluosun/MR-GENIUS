mm.binary <- function (b, n_iv, design.mat, exposure, iv, outcome, n_sample) {
	...
	if (n_iv>1) {
		...
		#IV equations
		iv.e = sweep(iv,2,apply(iv,2,mean),"-")*as.vector(exposure-expit(design.mat%*%b[1:dim_a]))*(outcome-b[dim_a+1]*exposure);
		...
		nr.e = design.mat*as.vector(exposure-expit(design.mat%*%b[1:dim_a]));
		#IV equations
		iv.e = (iv-mean(iv))*as.vector(exposure-expit(design.mat%*%b[1:dim_a]))*(outcome-b[dim_a+1]*exposure);
		#centering the IV moment conditions
		iv.e = iv.e-mean(iv.e);
		tildem = cbind(mu.e, nr.e, iv.e);
	}
...
dudbeta.binary <- function (b, n_iv, design.mat, exposure, iv, outcome, n_sample) {
	dim_a=length(b)-1;
	db = -sweep(iv,2,apply(iv,2,mean),"-")*as.vector(exposure-expit(design.mat%*%b[1:dim_a]))*exposure;
	t(apply(db,2,mean))
}
...
B.binary <- function (b, n_iv, design.mat, exposure, iv, outcome, n_sample) {
	...
	if (n_iv>1) {
		A_prime = dudbeta.binary(b, n_iv, design.mat, exposure, iv, outcome, n_sample);
		Omega_inv = W.opt.binary(b, n_iv, design.mat, exposure, iv, outcome, n_sample); 
		dudmu = -diag(n_iv)*mean(as.vector(exposure-expit(design.mat%*%b[1:dim_a]))*(outcome-b[dim_a+1]*exposure));
		dudpsi = -(1/n_sample)*t(sweep(iv,2,apply(iv,2,mean),"-")*(outcome-b[dim_a+1]*exposure))%*%(pvar*design.mat);
		B[(dim_a+n_iv+1),] = (A_prime%*%Omega_inv)%*%cbind(dudmu, dudpsi,t(A_prime));
	}
	else {
		dudmu = -1*mean(as.vector(exposure-expit(design.mat%*%b[1:dim_a]))*(outcome-b[dim_a+1]*exposure));
		dudpsi= -1*apply( (iv-mean(iv))*(outcome-b[dim_a+1]*exposure)*(pvar*design.mat) ,2, mean);
		dudb  = -mean((iv-mean(iv))*as.vector(exposure-expit(design.mat%*%b[1:dim_a]))*exposure);
